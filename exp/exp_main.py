...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if hasattr(selected_model, 'Model'):
            ...
            # Nếu không có thuộc tính Model, giả định rằng nó đã là một lớp model
    ...
    def _process_batch(self, batch, batch_idx=None, data_set=None):
        ...
        return batch_x, batch_y, batch_x_mark, batch_y_mark, text_descriptions
    ...
    def _compute_loss(self, batch_x, batch_y, batch_x_mark, batch_y_mark, dec_inp, criterion, text_descriptions=None):
        if self.args.is_pretrain:
            ...
            # Pass text descriptions to the model if available and the model is LLMPatchTST
            if self.args.model == 'LLMPatchTST' and text_descriptions is not None:
                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, text_descriptions)
            else:
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, batch in enumerate(vali_loader):
                batch_x, batch_y, batch_x_mark, batch_y_mark, text_descriptions = self._process_batch(batch, i,
                                                                                                      vali_data)
                # decoder input
            ...
            for i, batch in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            if self.args.model == 'LLMPatchTST' and text_descriptions is not None:
                                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, text_descriptions)[0]
                            else:
